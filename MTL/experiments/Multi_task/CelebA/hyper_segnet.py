...
class SegNetTarget(nn.Module):
    ...
    def forward(self, x, weights=None):
        ...
        for i in range(5):
            if i == 0:
                ...
                g_decoder[i][0] = self.decoder_block(g_upsampl[i],weights,-i-1)
                g_decoder[i][1] = self.conv_block_dec(g_decoder[i][0],weights,-i-1)
            else:
        ...
        return [t1_pred, t2_pred, t3_pred], g_decoder[i][1]
    ...
    def forward_clf(self,x, weights, index):
        ...
        x = F.conv2d(x, weights[f"segnet.pred_task{index}.conv2d2.weight"], stride=1, padding=1)
        return x