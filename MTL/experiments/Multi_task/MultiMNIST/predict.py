...
def phn_test(dataset,path_data,save_weights,device,mode):
    ...
    hnet1 = LeNetHyper_trans([9, 5], ray_hidden_dim=hidden_dim)
    ...
    # Load checkpoints
    ckpt = torch.load(os.path.join(save_weights,'best_model_cheby_multi_'+str(dataset)+'_mlp.pkl'),map_location=device)
    #print(ckpt)
    hnet.load_state_dict(ckpt['state_dicts'])
    ckpt1 = torch.load(os.path.join(save_weights,'best_model_cheby_multi_'+str(dataset)+'_trans.pkl'),map_location=device)
    hnet1.load_state_dict(ckpt1['state_dicts'])
    # ckpt2 = torch.load(os.path.joite_dicts'])
    # ckpt2 = torch.load(os.path.joite_dicts'])
    # ckpt2 = torch.load(os.path.joite_dicts'])
    # ckpt2 = torch.load(os.path.joite_dicts'])
    # ckpt2 = torch.load(os.path.joite_dicts'])
    ...
    #n(save_weights,'best_model_cheby_multi_'+str(dataset)+'.pkl'),map_location=device)
    # hnet2.load_state_dict(ckpt2['state_dicts'])
    # ckpt3 = torch.load(os.path.join(save_weights,'best_model_epo_multi_'+str(dataset)+'.pkl'),map_location=device)
    # hnet3.load_state_dict(ckpt3['state_dicts'])
    # hnet = hnet.to(device)
    ...
    if mode == "loss":
        if dataset == 'mnist':
            ...
            #plt.scatter(x_, y_,marker='*',label = 'MLP')
            ...
            #plt.scatter(x1_, y1_,marker='o',label = 'Transfomer')
        ...
        print("HV Trans: ",hypervolumn(np.array(loss_hv1), type='loss', ref=np.ones(2) * 2))
...
dataset = 'mnist'
path_data = '/home/tuantran/PHN-CSF/MTL/experiments/dataset/Multi_task/MultiMNIST'
